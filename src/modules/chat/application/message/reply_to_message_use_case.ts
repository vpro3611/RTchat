...
export class ReplyToMessageUseCase {
    constructor(private readonly messageRepo: MessageRepoInterface,
                private readonly messageReplyRepo: MessageReplyRepoInterface,
    ...
    async replyToMessageUseCase(actorId: string, conversationId: string, parentMessageId: string, content: string, files: FileDTO[] = []): Promise<MessageDTO> {
        ...
        if (!parentMessage) {
            throw new MessageNotFoundError("Parent message not found");
        }
        ...
        await this.messageReplyRepo.create({
