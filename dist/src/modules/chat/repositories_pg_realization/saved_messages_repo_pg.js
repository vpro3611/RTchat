...
exports.SavedMessagesRepoPg = void 0;
...
class SavedMessagesRepoPg {
    pool;
    constructor(pool) {
        this.pool = pool;
    }
    mapToSavedMessages(row) {
        return saved_messages_1.SavedMessages.restore(row.saved_by_user, row.message_id, row.conversation_id, row.sender_id, row.content, row.created_at, row.updated_at);
    }
    async saveMessage(savedMessage) {
        try {
            const query = `INSERT INTO saved_messages
    ...
    async getSavedMessages(savedById, limit = 20, cursor) {
        try {
            const params = [savedById];
            let cursorCondition = "";
            if (cursor) {
            ...
            const items = rows.map((row) => this.mapToSavedMessages(row));
    ...
    async removeSavedMessage(messageId, savedById) {
        try {
            const query = `DELETE FROM saved_messages
    ...
    async isSaved(messageId, savedById) {
        try {
            const query = `SELECT EXISTS (SELECT 1 FROM saved_messages WHERE message_id = $1 AND saved_by_user = $2) AS exists`;
    ...
    async getSpecificSavedMessage(messageId, savedById) {
        try {
            const query = `
...
exports.SavedMessagesRepoPg = SavedMessagesRepoPg;
