...
const cache_service_1 = require("../../src/modules/infrasctructure/ports/cache_service/cache_service");
describe("CacheService", () => {
    ...
    beforeEach(() => {
        ...
        cache = new cache_service_1.CacheService(redis);
    });
    // =========================
    // get
    // =========================
    ...
    it("should set value with ttl", async () => {
        ...
        expect(redis.set).toHaveBeenCalledWith("key", JSON.stringify({ a: 1 }), { EX: 120 });
    });
    // =========================
    // update
    // =========================
    it("should update value with ttl", async () => {
        ...
        expect(redis.set).toHaveBeenCalledWith("key", JSON.stringify({ a: 2 }), { EX: 300 });
    });
    it("should update value without ttl", async () => {
        await cache.update("key", { a: 2 });
        expect(redis.set).toHaveBeenCalledWith("key", JSON.stringify({ a: 2 }));
    });
    // =========================
    // del
    // =========================
