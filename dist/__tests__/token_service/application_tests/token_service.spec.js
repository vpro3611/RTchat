...
const token_service_1 = require("../../../src/modules/authentification/jwt_token_service/token_service");
...
describe("TokenServiceJWT", () => {
    ...
    beforeEach(() => {
        ...
        service = new token_service_1.TokenServiceJWT();
    });
    afterAll(() => {
        process.env = originalEnv;
    });
    ...
    it("should throw InvalidTokenJWTError for invalid access token", () => {
        expect(() => service.verifyAccessToken("invalid.token.here")).toThrow(token_errors_1.InvalidTokenJWTError);
    });
    it("should throw InvalidTokenJWTError for invalid refresh token", () => {
        expect(() => service.verifyRefreshToken("invalid.token.here")).toThrow(token_errors_1.InvalidTokenJWTError);
    });
    // ---------------------------
    // SECRET NOT DEFINED
    // ---------------------------
    it("should throw if ACCESS_TOKEN_SECRET not defined", () => {
        ...
        expect(() => service.generateAccessToken(userId)).toThrow(token_errors_1.SecretNotDefinedError);
    });
    it("should throw if REFRESH_TOKEN_SECRET not defined", () => {
        delete process.env.REFRESH_TOKEN_SECRET;
        expect(() => service.generateRefreshToken(userId)).toThrow(token_errors_1.SecretNotDefinedError);
    });
    // ---------------------------
