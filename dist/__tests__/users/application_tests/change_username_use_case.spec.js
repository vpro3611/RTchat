...
const change_username_use_case_1 = require("../../../src/modules/users/application/change_username_use_case");
...
describe("ChangeUsernameUseCase", () => {
    ...
    beforeEach(() => {
        ...
        useCase = new change_username_use_case_1.ChangeUsernameUseCase(reader, writer, mapper, userLookup);
        user = user_1.User.restore("11111111-1111-1111-1111-111111111111", "oldusername", "test@example.com", "hash", true, true, new Date(), new Date(), new Date());
    });
    it("should change username successfully", async () => {
        ...
        const result = await useCase.changeUsernameUseCase(user.id, "newusername");
    ...
    it("should throw if username already exists", async () => {
        ...
        await expect(useCase.changeUsernameUseCase(user.id, "existingusername")).rejects.toBeInstanceOf(username_errors_1.UsernameAlreadyExistsError);
        expect(writer.save).not.toHaveBeenCalled();
        expect(mapper.mapToDto).not.toHaveBeenCalled();
    });
    it("should throw if user is inactive", async () => {
        const inactiveUser = user_1.User.restore(user.id, "oldusername", "test@example.com", "hash", false, true, new Date(), new Date(), new Date());
        userLookup.getUserOrThrow.mockResolvedValue(inactiveUser);
        reader.getUserByUsername.mockResolvedValue(null);
        await expect(useCase.changeUsernameUseCase(user.id, "newusername")).rejects.toThrow();
        expect(writer.save).not.toHaveBeenCalled();
    });
    it("should throw if user is not verified", async () => {
        const notVerifiedUser = user_1.User.restore(user.id, "oldusername", "test@example.com", "hash", true, false, new Date(), new Date(), new Date());
    ...
    it("should throw if username is invalid", async () => {
        await expect(useCase.changeUsernameUseCase(user.id, "ab")).rejects.toThrow();
    ...
    it("should propagate lookup error", async () => {
        ...
        await expect(useCase.changeUsernameUseCase(user.id, "newusername")).rejects.toThrow("USER_NOT_FOUND");
        expect(writer.save).not.toHaveBeenCalled();
    });
});
