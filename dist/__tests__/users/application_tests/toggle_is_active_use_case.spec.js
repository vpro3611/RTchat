...
const toggle_status_use_case_1 = require("../../../src/modules/users/application/toggle_status_use_case");
...
describe("ToggleIsActiveUseCase", () => {
    ...
    beforeEach(() => {
        ...
        useCase = new toggle_status_use_case_1.ToggleIsActiveUseCase(writer, mapper, lookup);
    });
    function createUser(isActive, isVerified = true) {
        return user_1.User.restore("11111111-1111-1111-1111-111111111111", "username", "test@example.com", "hash", isActive, isVerified, new Date(), new Date(), new Date());
    }
    ...
    it("should throw if user is not verified", async () => {
        ...
        lookup.getUserOrThrow.mockResolvedValue(user);
        await expect(useCase.toggleIsActiveUseCase(user.id)).rejects.toThrow();
    ...
    it("should propagate error if lookup fails", async () => {
        lookup.getUserOrThrow.mockRejectedValue(new Error("USER_NOT_FOUND"));
        await expect(useCase.toggleIsActiveUseCase("invalid-id")).rejects.toThrow("USER_NOT_FOUND");
        expect(writer.save).not.toHaveBeenCalled();
    });
    it("should propagate error if save fails", async () => {
        const user = createUser(true, true);
        lookup.getUserOrThrow.mockResolvedValue(user);
        writer.save.mockRejectedValue(new Error("DATABASE_ERROR"));
        await expect(useCase.toggleIsActiveUseCase(user.id)).rejects.toThrow("DATABASE_ERROR");
        expect(mapper.mapToDto).not.toHaveBeenCalled();
    });
});
