...
const block_specific_user_use_case_1 = require("../../../src/modules/users/application/block_specific_user_use_case");
...
describe("BlockSpecificUserUseCase", () => {
    ...
    beforeEach(() => {
        ...
        useCase = new block_specific_user_use_case_1.BlockSpecificUserUseCase(userToUserBlocksRepo, userRepoReader, userMapper);
    });
    // -------------------------
    // SUCCESS
    // -------------------------
    ...
    it("should throw CannotBlockYourselfError when blocking yourself", async () => {
        await expect(useCase.blockSpecificUserUseCase(actorId, actorId)).rejects.toBeInstanceOf(use_case_errors_1.CannotBlockYourselfError);
    ...
    it("should throw when actor not found", async () => {
        ...
        await expect(useCase.blockSpecificUserUseCase(actorId, targetId)).rejects.toThrow("User not found");
        expect(userToUserBlocksRepo.blockSpecificUser).not.toHaveBeenCalled();
    });
    ...
    it("should throw when actor is not verified", async () => {
        ...
        userRepoReader.getUserById.mockResolvedValue(mockUser);
        await expect(useCase.blockSpecificUserUseCase(actorId, targetId)).rejects.toThrow("not active or not verified");
        expect(userToUserBlocksRepo.blockSpecificUser).not.toHaveBeenCalled();
    });
    ...
    it("should throw when user already blocked", async () => {
        ...
        await expect(useCase.blockSpecificUserUseCase(actorId, targetId)).rejects.toThrow("Failed to block user, user already blocked by you");
        expect(userToUserBlocksRepo.blockSpecificUser).not.toHaveBeenCalled();
    });
    // -------------------------
    // EDGE CASES
    ...
    it("should propagate repository error", async () => {
        ...
        await expect(useCase.blockSpecificUserUseCase(actorId, targetId)).rejects.toThrow("DB error");
    });
    it("should handle invalid UUID format", async () => {
        // UUID validation happens at controller level, not use case
        // This test documents the expected behavior
        await expect(useCase.blockSpecificUserUseCase("invalid-uuid", targetId)).rejects.toThrow();
    });
});
