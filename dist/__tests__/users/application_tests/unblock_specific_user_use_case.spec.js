...
const unblock_specific_user_use_case_1 = require("../../../src/modules/users/application/unblock_specific_user_use_case");
...
describe("UnblockSpecificUserUseCase", () => {
    ...
    beforeEach(() => {
        ...
        useCase = new unblock_specific_user_use_case_1.UnblockSpecificUserUseCase(userToUserBlocksRepo, userRepoReader, userMapper);
    });
    // -------------------------
    // SUCCESS
    // -------------------------
    ...
    it("should throw CannotBlockYourselfError when unblocking yourself", async () => {
        await expect(useCase.unblockSpecificUserUseCase(actorId, actorId)).rejects.toBeInstanceOf(use_case_errors_1.CannotBlockYourselfError);
    ...
    it("should throw when actor not found", async () => {
        ...
        await expect(useCase.unblockSpecificUserUseCase(actorId, targetId)).rejects.toThrow("User not found");
        expect(userToUserBlocksRepo.unblockSpecificUser).not.toHaveBeenCalled();
    });
    // -------------------------
    ...
    it("should throw when actor is not verified", async () => {
        ...
        userRepoReader.getUserById.mockResolvedValue(mockUser);
        await expect(useCase.unblockSpecificUserUseCase(actorId, targetId)).rejects.toThrow("not active or not verified");
        expect(userToUserBlocksRepo.unblockSpecificUser).not.toHaveBeenCalled();
    });
    ...
    it("should throw when user is not blocked", async () => {
        ...
        await expect(useCase.unblockSpecificUserUseCase(actorId, targetId)).rejects.toThrow("Failed to unblock user, user not blocked by you");
        expect(userToUserBlocksRepo.unblockSpecificUser).not.toHaveBeenCalled();
    });
    // -------------------------
    // EDGE CASES
    ...
    it("should propagate repository error", async () => {
        ...
        await expect(useCase.unblockSpecificUserUseCase(actorId, targetId)).rejects.toThrow("DB error");
    });
    it("should handle invalid UUID format", async () => {
        await expect(useCase.unblockSpecificUserUseCase("invalid-uuid", targetId)).rejects.toThrow();
    });
});
