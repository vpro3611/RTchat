...
const user_repo_writer_pg_1 = require("../../../src/modules/users/repositories/user_repo_writer_pg");
...
describe("UserRepoWriterPg (integration - transactional)", () => {
    ...
    beforeEach(async () => {
        ...
        repo = new user_repo_writer_pg_1.UserRepoWriterPg(client);
    });
    afterEach(async () => {
    ...
    function createTestUser(overrides) {
        return user_1.User.restore(overrides?.id ?? "11111111-1111-1111-1111-111111111111", overrides?.username ?? "testuser", overrides?.email ?? "test@example.com", overrides?.passwordHash ?? "hash123", overrides?.is_active ?? true, overrides?.is_verified ?? true, overrides?.last_seen_at ?? new Date(), overrides?.created_at ?? new Date(), overrides?.updated_at ?? new Date());
    }
    ...
    it("should update existing user (upsert)", async () => {
        ...
        const result = await client.query("SELECT username FROM users WHERE id = $1", [user.id]);
    ...
    it("should throw DATABASE_QUERY_ERROR", async () => {
        const user = createTestUser({
            id: "invalid-query",
    ...
    it("should mark user as verified", async () => {
        ...
        const result = await client.query("SELECT is_verified FROM users WHERE id = $1", [user.id]);
