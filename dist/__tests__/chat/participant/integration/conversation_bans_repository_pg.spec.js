...
const conversation_bans_repository_pg_1 = require("../../../../src/modules/chat/repositories_pg_realization/conversation_bans_repository_pg");
describe("ConversationBansRepositoryPg (integration)", () => {
    ...
    beforeEach(async () => {
        ...
        repo = new conversation_bans_repository_pg_1.ConversationBansRepositoryPg(client);
        // users
        await client.query(`
    ...
    function createBan(userId, bannedBy, reason) {
        return {
    ...
    describe("ban", () => {
        it("should ban user successfully", async () => {
            ...
            await repo.ban(ban);
            const result = await client.query(`SELECT * FROM conversation_bans WHERE conversation_id = $1 AND user_id = $2`, [CONVERSATION_ID, USER_B]);
        ...
        it("should allow banning same user in different conversations", async () => {
            ...
            const result1 = await client.query(`SELECT * FROM conversation_bans WHERE conversation_id = $1`, [CONVERSATION_ID]);
            const result2 = await client.query(`SELECT * FROM conversation_bans WHERE conversation_id = $1`, [CONV2]);
    ...
    describe("unban", () => {
        ...
        it("should not throw when trying to unban non-banned user", async () => {
            await expect(repo.unban(CONVERSATION_ID, USER_C)).resolves.not.toThrow();
        });
        it("should only unban specific user in specific conversation", async () => {
            ...
            await repo.ban(banC);
            await repo.unban(CONVERSATION_ID, USER_B);
            const resultB = await client.query(`SELECT * FROM conversation_bans WHERE user_id = $1`, [USER_B]);
            const resultC = await client.query(`SELECT * FROM conversation_bans WHERE user_id = $1`, [USER_C]);
