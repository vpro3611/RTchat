...
const ban_group_participant_service_1 = require("../../../../src/modules/chat/transactional_services/participant/ban_group_participant_service");
describe("BanGroupParticipantService (transaction)", () => {
    ...
    beforeEach(() => {
        ...
        service = new ban_group_participant_service_1.BanGroupParticipantService(txManager);
    });
    // =========================
    ...
    it("should call runInTransaction when banning participant", async () => {
        ...
        await expect(service.banGroupParticipantService(CONVERSATION_ID, TARGET_ID, ACTOR_ID, "Spam")).rejects.toThrow(); // Will fail because repos aren't mocked, but tx is called
    ...
    it("should pass correct parameters to transaction callback", async () => {
        ...
        txManager.runInTransaction = jest.fn(async (callback) => {
        ...
        try {
            await service.banGroupParticipantService(CONVERSATION_ID, TARGET_ID, ACTOR_ID, "Spam");
        }
        catch (e) {
    ...
    it("should propagate transaction errors", async () => {
        ...
        await expect(service.banGroupParticipantService(CONVERSATION_ID, TARGET_ID, ACTOR_ID, "Spam")).rejects.toThrow("Transaction failed");
    });
});
