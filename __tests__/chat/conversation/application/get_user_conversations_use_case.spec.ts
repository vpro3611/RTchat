import { GetUserConversationsUseCase } from "../../../../src/modules/chat/application/conversation/get_user_conversations_use_case";
...
describe("GetUserConversationsUseCase", () => {
    ...
    let useCase: GetUserConversationsUseCase;
    ...
    beforeEach(() => {
        ...
        useCase = new GetUserConversationsUseCase(
            conversationRepo,
            mapper,
            cacheService
        );
    ...
    it("should generate correct cache key", async () => {
        ...
        const expectedKey =
            `conv:user:${USER_ID}:limit:10:cursor:cursor1`;
    ...
    it("should use default limit 20 when limit not provided", async () => {
        ...
        expect(cacheService.remember).toHaveBeenCalledWith(
            `conv:user:${USER_ID}:limit:20:cursor:start`,
            60,
            expect.any(Function)
        );
