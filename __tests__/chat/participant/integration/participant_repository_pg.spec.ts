...
import { ParticipantRepositoryPg } from "../../../../src/modules/chat/repositories_pg_realization/participant_repository_pg";
...
describe("ParticipantRepositoryPg (integration)", () => {
    ...
    let repo: ParticipantRepositoryPg;
    ...
    function createParticipant(userId: string) {
        return Participant.createAsMember(
            CONVERSATION_ID,
            userId
        );
    }
    ...
    it("should update participant on conflict", async () => {
        ...
        const result = await client.query(
            `SELECT can_send_messages FROM conversation_participants WHERE user_id=$1`,
            [USER_A]
        );
    ...
    it("should return null if participant not found", async () => {
        ...
        const found = await repo.findParticipant(
            CONVERSATION_ID,
            USER_A
        );
