import { GetFullBlackListUseCase } from "../../../src/modules/users/application/get_full_black_list_use_case";
import { User } from "../../../src/modules/users/domain/user";

describe("GetFullBlackListUseCase", () => {
    let userToUserBlocksInterface: any;
    let userLookup: any;
    let userMapper: any;
    let useCase: GetFullBlackListUseCase;
    ...
    beforeEach(() => {
        ...
        useCase = new GetFullBlackListUseCase(
            userToUserBlocksInterface,
            userLookup,
            userMapper
        );
    ...
    it("should throw when user not found", async () => {
        ...
        await expect(
            useCase.getFullBlackListUseCase(actorId)
    ...
    it("should handle large blacklist", async () => {
        const mockUser = createMockUser(actorId, true, true);
        const blockedUsers = Array.from({ length: 100 }, (_, i) =>
            createMockUser(`22222222-2222-2222-2222-${i.toString().padStart(12, '0')}`, true, true)
        );

        userLookup.getUserOrThrow.mockResolvedValue(mockUser);
        userToUserBlocksInterface.getFullBlacklist.mockResolvedValue(blockedUsers);
        userMapper.mapToDto.mockImplementation((user: User) => ({ id: user.id }));
